...
function AuthorProfile() {
  ...
  //Call this function on lgout
  const onLogout = async () => {
    //Call login route
    await logout();
    //navigate to login page again after logout
    navigate("/login");
  };
  ...
  return (
    <div className={pageWrapper}>
      ...
      <div className="bg-white border border-[#e8e8ed] rounded-3xl p-6 mb-8 shadow-sm flex items-center justify-between">
        ...
        {/* LOGOUT */}
        <button
          className="bg-purple-600 text-white text-sm px-5 py-2 rounded-full hover:bg-purple-800 transition"
          onClick={onLogout}
        >
          Logout
        </button>
      ...
      <div className="flex gap-3 mb-6 bg-[#f5f5f7] p-2 rounded-full w-fit">
        <NavLink
          ...
          className={({ isActive }) =>
            isActive
              ? "bg-white px-3 py-3 rounded-full text-purple-900 font-medium shadow-sm"
        ...
        <NavLink
          ...
          className={({ isActive }) =>
            isActive
              ? "bg-white px-3 py-3 rounded-full  text-purple-900 text-md font-medium shadow-sm"
              : `${navLinkClass} px-5 py-2`
          }
        >
          Write Article
